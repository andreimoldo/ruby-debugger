...
export default class Client {
  ...
  frames() {
    return this.runCmdWithResponse('where');
  }
  ...
  removeBreakpoint(breakpointId) {
    return this.runCmdWithResponse('delete', breakpointId);
  }

  enableBreakpoint(breakpointId) {
    return this.runCmdWithResponse('enable', breakpointId);
  }

  disableBreakpoint(breakpointId) {
    return this.runCmdWithResponse('disable', breakpointId);
  }
  ...
  handleCmd(command) {
    ...
    data = command[name];
    method = "handle" + _.upperFirst(name) + "Cmd";
  ...
  handleBreakpointDeletedCmd(data) {
    var num = data.attrs.no;
    this.deferreds.shift().resolve(data);
    return this.events.emit('breakpointDeleted', num);
  }

  handleBreakpointEnabledCmd(data) {
    var num = data.attrs.bp_id;
    this.deferreds.shift().resolve(data);
    return this.events.emit('breakpointEnabled', num);
  }

  handleBreakpointDisabledCmd(data) {
    var num = data.attrs.bp_id;
    this.deferreds.shift().resolve(data);
    return this.events.emit('breakpointDisabled', num);
  }
